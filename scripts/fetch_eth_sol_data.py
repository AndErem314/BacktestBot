#!/usr/bin/env python3
"""
Fill ETH and SOL data from Binance using CCXT.
Follows the same pattern as fill_crypto_data_gap.py for BTC.
"""
import sys
...
def fill_data_gap(symbol_short='ETH', timeframe='4h'):
    ...
    # Initialize data fetcher (same as BTC script)
    ...
    if result and result[0]:
        ...
        print(f"No existing data found for {timeframe}, will fetch from 2020-01-01")
        since_dt = pd.to_datetime('2020-01-01')
...
if __name__ == "__main__":
    print("Starting data fill for ETH and SOL...")
    
    for symbol in ['ETH', 'SOL']:
        # Fill 4h data
        fill_data_gap(symbol, '4h')
        # Fill 1d data
        fill_data_gap(symbol, '1d')
    
    print(f"\n{'='*60}")
    print("✅ ETH and SOL data fill complete!")
    print(f"{'='*60}")
